package com.mygdx.application.input_manager;
...
import com.mygdx.services.vocal_recognition.SpeechRecognition;
...
public class InputManager implements InputProcessor {
	...
	public boolean keyDown(int keycode) {
		...
		if (Housim.checkRoom().equals("mainRoom") || Housim.checkRoom().equals("bathroom")) {
			if (Housim.checkRoom().equals("mainRoom")) {
				if (keycode == Input.Keys.T) {
					if (isTvOn) {
						Housim.getHousim().getCommand().tvOff();
					}else {
						Housim.getHousim().getCommand().tvOn();
					}
				}
				if (keycode == Input.Keys.L) {
					if (isLightOn) {
						Housim.getHousim().getCommand().lightOff();
					}else {
						Housim.getHousim().getCommand().lightOn();
					}
				}

				if (keycode == Input.Keys.B) {
					if (activateSpeaker) {
						Housim.getHousim().getCommand().speakerOff();
					}else {
						Housim.getHousim().getCommand().speakerOn();
					}
				}

			}

			if (Housim.checkRoom().equals("bathroom")) {
				if (keycode == Input.Keys.V) {
					if (!activateFan) {
						Housim.getHousim().getCommand().fanOn();
					}
				}
				...
				if (keycode == Input.Keys.C) {
					if (safeBox) {
						Housim.getHousim().getCommand().safeBoxClose();
					}else {
						Housim.getHousim().getCommand().safeBoxOn();
					}
				}
